...
async def on_afk(event):
    ...
    if USERAFK_ON and not (await event.get_sender()).bot:
        msg = None
        message_to_reply = (
            f"**👋Ciao sono afk non spammare o floodare ti risponderò presto**"
        )
...
@borg.on(admin_cmd(pattern=r"afk22 ?(.*)", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    global USERAFK_ON
    ...
    if not USERAFK_ON:
        ...
        if reason:
            await borg.send_message(
                event.chat_id, f"**✅Afk on✅**"
            )
        else:
            await borg.send_message(event.chat_id, f"**✅Afk on✅**")
        await asyncio.sleep(5)
        await event.delete()
        if BOTLOG:
            await borg.send_message(
